        ...
        _.forEach(cache, function (executions, itemId) {
            ...
            _.forEach(executions, function (execution) {
                ...
                _.forEach(execution.assertions, function (assertion) {
                    var testCaseName = util.getFullName(currentItem)
                    var testCaseNameArray = testCaseName.split("/")
                    var name = testCaseNameArray[testCaseNameArray.length -1] + " - " + assertion.assertion,
                        err = assertion.error;
                    if (err) {
                        ++failures;
            ...
            _.forOwn(tests, function (failures, name) {
                ...
                if (failures && failures.length) {
                    testcase.att('classname', 'JUnitXmlReporter.constructor');
